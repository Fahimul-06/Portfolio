...
export function ResearchManager() {
  ...
  return <div className="space-y-6">
    <div className="flex items-center justify-between gap-4">
      ...
      <button onClick={()=>{ reset(); setShowForm(true); }} className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2.5 font-semibold text-slate-950"><Plus size={18}/> Add Research</button>
    </div>
    {error && <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-red-300">{error}</div>}
    {showForm && <form onSubmit={submit} className="grid gap-4 rounded-2xl border border-slate-800 bg-slate-900 p-5 md:grid-cols-2">
      ...
      <input required placeholder="Research title" value={form.title} onChange={e=>setForm({...form,title:e.target.value})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white md:col-span-2" />
      <input placeholder="Authors, comma separated" value={form.authors} onChange={e=>setForm({...form,authors:e.target.value})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white" />
      <input placeholder="Journal / Conference" value={form.journal} onChange={e=>setForm({...form,journal:e.target.value})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white" />
      <input placeholder="Publication date (e.g. Apr 2026)" value={form.publication_date} onChange={e=>setForm({...form,publication_date:e.target.value})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white" />
      <input type="number" placeholder="Display order" value={form.display_order} onChange={e=>setForm({...form,display_order:Number(e.target.value)})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white" />
      <textarea placeholder="Abstract / summary" rows={6} value={form.abstract} onChange={e=>setForm({...form,abstract:e.target.value})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white md:col-span-2" />
      <input placeholder="Keywords, comma separated" value={form.keywords} onChange={e=>setForm({...form,keywords:e.target.value})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white md:col-span-2" />
      <input placeholder="DOI URL" value={form.doi_url} onChange={e=>setForm({...form,doi_url:e.target.value})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white" />
      <input placeholder="Paper / publisher URL" value={form.paper_url} onChange={e=>setForm({...form,paper_url:e.target.value})} className="rounded-xl border border-slate-700 bg-slate-950 p-3 text-white" />
      ...
      <div className="md:col-span-2 flex gap-3"><button disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-5 py-2.5 font-bold text-slate-950"><Save size={18}/>{saving?'Saving...':'Save'}</button><button type="button" onClick={reset} className="rounded-xl bg-slate-800 px-5 py-2.5">Cancel</button></div>
    </form>}
    <div className="space-y-3">{items.map(r=><div key={r.id} className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900 p-5 md:flex-row md:items-center">
      ...
      <div className="flex gap-2"><button onClick={()=>edit(r)} className="rounded-lg bg-slate-800 p-2 text-gray-300"><Edit size={18}/></button><button onClick={()=>remove(r.id)} className="rounded-lg bg-red-500/10 p-2 text-red-300"><Trash2 size={18}/></button></div>
    </div>)}</div>
  </div>;
}
