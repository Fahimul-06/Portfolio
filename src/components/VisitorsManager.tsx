    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-2 rounded-lg bg-violet-500 px-4 py-2 font-semibold text-[#0b0614] transition hover:bg-violet-400 disabled:opacity-70"
        >
      ...
      {loading && visitors.length === 0 ? (
        <div className="rounded-2xl border border-[#322044] bg-[#160b24] p-8 text-center text-gray-400">Loading visitors...</div>
      ) : visitors.length === 0 ? (
        <div className="rounded-2xl border border-[#322044] bg-[#160b24] p-8 text-center text-gray-400">No visitors tracked yet.</div>
      ) : (
        <div className="space-y-4">
          {visitors.map((visitor) => {
            ...
            return (
              <div key={visitor.id} className="overflow-hidden rounded-2xl border border-[#322044] bg-[#160b24] shadow-xl">
                <div className="p-5">
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {gpsMap && (
                        <a href={gpsMap} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-green-500 px-3 py-2 text-sm font-semibold text-[#0b0614] hover:bg-green-400">
                          <Navigation size={16} /> Exact map
                        </a>
                      )}
                      {ipMap && (
                        <a href={ipMap} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-[#452c5d] px-3 py-2 text-sm font-semibold text-gray-300 hover:bg-[#211032]">
                          <Globe2 size={16} /> Approx map
                        </a>
                      )}
                      <button
                        onClick={() => setExpanded(isExpanded ? null : visitor.id)}
                        className="inline-flex items-center gap-2 rounded-lg border border-[#452c5d] px-3 py-2 text-sm font-semibold text-gray-300 hover:bg-[#211032]"
                      >
                ...
                {isExpanded && (
                  <div className="border-t border-[#322044] bg-[#0b0614]/50 p-5">
                    <div className="grid gap-4 lg:grid-cols-2">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <p className="break-words rounded-xl border border-[#322044] bg-[#160b24] p-3 text-xs text-gray-400">{visitor.user_agent || '—'}</p>
